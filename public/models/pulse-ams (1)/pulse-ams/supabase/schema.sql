...
create table if not exists employees (
  ...
  face_descriptor jsonb,                -- 128-value face-api.js embedding, not a photo
...
-- SECURITY DEFINER + a fixed search_path makes this function's internal
-- query bypass RLS on profiles, instead of re-triggering the "admin
-- manages profiles" policy below (which itself calls this function) —
-- without this, checking that policy recurses infinitely.
